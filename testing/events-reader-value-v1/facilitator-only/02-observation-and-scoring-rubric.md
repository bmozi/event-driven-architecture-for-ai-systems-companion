...
| Entry-branch integrity | Exactly one manifest-verified human-consent or synthetic-context branch precedes scored work; a synthetic branch contains no fictional human consent or human-result claim | | |
| Stage-boundary integrity | Separate facilitator-log checkpoints bind Stage A/B starts and ends, Stage A material feedback, Stage B scoring end, debrief completion, and results completion; governed workbooks contain no predicted future end fields | | |
| Debrief integrity | Section 6 opens only after scoring ends and its exact input manifest verifies; debrief cannot modify or rescore frozen Sections 1-5 | | |
| Literal one-page proof | A favorable handoff claim has a preserved one-page US Letter PDF/proof with >=0.5-inch margins, >=9-point text, <=450 reader-facing words excluding labeled provenance, and no clipping/overlap/hidden overflow/shrinking; layout is not comprehension | | |
| Full-route closure | Six scored freeze chains are reported separately from entry, boundaries, debrief, immutable pre-close results, log close, and later external closeout | | |
